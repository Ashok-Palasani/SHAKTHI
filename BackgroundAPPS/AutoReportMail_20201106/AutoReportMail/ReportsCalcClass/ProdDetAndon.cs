       ...
       // DateTime CorrectedDate = Convert.ToDateTime("2020-07-13");
        ...
        public void InsertGraphData(int machineID, DateTime correctedDate)
        {
            ...
            if (GetModDet == null)
            {
                ...
                String corrdate = Convert.ToString(correctedDate.Date);
                using (i_facility_shaktiEntities Serverdb = new i_facility_shaktiEntities())
                {
                    GetModeDurations = Serverdb.tblmodes.Where(m => m.MachineID == machineID && m.CorrectedDate == corrdate && m.IsCompleted == 1).ToList();
                }

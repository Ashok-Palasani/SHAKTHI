...
namespace DAS.DAL
{
    public class DALPlantShopCellData : IPlantShopCellData
    {
        ...
        private static readonly log4net.ILog log = log4net.LogManager.GetLogger(typeof(DALPlantShopCellData));

        public DALPlantShopCellData(i_facility_shaktiContext _db)
        {
            db = _db;
        }
        ...
        public CommonResponse GetMachineDetails(int cellId)
        {
            ...
            try
            {
                var machineData = db.Tblmachinedetails.Where(x => x.IsDeleted == 0 && x.CellId == cellId).Select(x => new { x.MachineId, x.MachineName }).ToList();
                foreach (var row in machineData)
                {
                    MachineEntity objMachine = new MachineEntity();
                    objMachine.machineId = row.MachineId;
                    objMachine.machineName = row.MachineName;
        ...
        public CommonResponse GetPlantShopCellMachineNames(PlantShopCellMachineId data)
        {
            ...
            try
            {
                ...
                var machineData = db.Tblmachinedetails.Where(m => m.IsDeleted == 0 && m.MachineId == data.machineId).Select(m => m.MachineName).FirstOrDefault();

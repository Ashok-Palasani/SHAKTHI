...
namespace SRKSDAQFanuc
{
    public partial class Service1 : ServiceBase
    {
        ...
        protected override void OnStop()
        {
            //MsqlConnection mc = new MsqlConnection();
            //mc.open();
            //SqlCommand cmd = new SqlCommand("INSERT INTO  i_facility.operationlog(OpMsg,OpDate,OpTime,OpDateTime,OpReason) VALUES('SRKS DataLogging Service was by stopped','" + System.DateTime.Now.ToString("yyyy-MM-dd") + "','" + System.DateTime.Now.ToString("HH:mm:ss") + "','" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','M/c Error')");
            //cmd.ExecuteNonQuery();
            //mc.close();
        ...
        private void insertdb(object sender, System.Timers.ElapsedEventArgs e)
        {
            ...
            //if (CurDateTime <= DemoEndTime)
            {
                ...
                //string Shift = "";
                int shiftid = 0;
                shift_master shiftmasterdet = new shift_master();
                try
                {
                    shiftid = GetShift();
                ...
                for (int j = 0; j < count; j++)
                {
                    ...
                    Task.Factory.StartNew(() =>
                    {
                        ...
                        //{
                        //    mconn.open();
                        //    String getparametersquery = "SELECT InsertedOn From  parameters_master WHERE MachineID = " + mcid + " order by ParameterID DESC";
                        ...
                        //if (DateTime.Now.Subtract(PrvDT).TotalSeconds >= 60.0)
                        {
                            ...
                            IntoFile("ConnectionRetErr:" + ret + "machineid:" + mcid);
                            if (ret == 0)
                            {
                                ...
                                if (ret == 0)
                                {
                                    ...
                                    if (ParameterExcep != 1)
                                    {
                                        partcount = Convert.ToDouble(parcout.rdata.prm_val.ToString() + "." + parcout.rdata.dec_val.ToString());
                                        parttotal = Convert.ToDouble(partot.rdata.prm_val.ToString() + "." + partot.rdata.dec_val.ToString());
                                    }
                                    else if (ParameterExcep == 1 || partcount == 0) //Get partcount for parameter exception machines and parcount as 0 machiens
                                    {
                                        string IsParameter_partcount = GetPartCount(h);  //Get partcount for parameter exception machines;
                                        partcount = Convert.ToDouble(IsParameter_partcount);
                                        parttotal = partcount;
                                    }
                                    IntoFile("operatingtime: " + operatingtime + "and cuttingtime:" + cuttingtime);
                                    ...
                                    //if (operatingtime != 0 && powertime != 0)
                                    {
                                        ...
                                        if (cuttingtime != 0)
                                        {
                                            ...
                                            try
                                            {
                                                parameters_master obj = new parameters_master();
                                                ...
                                                using (i_facility_shaktiEntities db = new i_facility_shaktiEntities())
                                                {
                                                    db.parameters_master.Add(obj);
                                                    db.SaveChanges();
                                                    IntoFile("parameters_master Insert Done");
                                                }
                                            }
                                            catch (Exception ex)
                                            {
                                                IntoFile("parameters_master Insert Exception:" + ex.ToString());
                                            }
                            ...
                            //if (ret == 0 || (ret != 0 && diff > 2))
                            {
                                try
                                {
                                    #region LOCK the machine if there is no Workorders on the Operator entry
                                    string corrctdt = GetCorrectedDate().ToString();
                                    int workorderscount = 0;
                                    tbllivemode brkdwnmd = new tbllivemode();
                                    using (i_facility_shaktiEntities db = new i_facility_shaktiEntities())
                                    {

                                        workorderscount = db.tbllivehmiscreens.Where(m => m.CorrectedDate == corrctdt && m.MachineID == mcid && m.Date == null && m.Time == null).OrderByDescending(m => m.HMIID).ToList().Count;
                                        brkdwnmd = db.tbllivemodes.Where(m => m.IsCompleted == 0 && m.MachineID == mcid && m.ColorCode == "RED").FirstOrDefault();
                                    }
                                ...
                                {
                                    IntoFile("diff error" + ex.ToString());
                            ...
                            if (counter5 == -1 || diffAlarm > 5)
                            {
                                ...
                                if (ret == 0)
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        if (type == 1)
                                        {
                                            ...
                                            if (stopophisret == 0)
                                            {
                                                ...
                                                IntoFile("insertAlarmHIsDB:" + mcid);
                                                try
                                                {
                                    ...
                                    //    SqlCommand cmd = new SqlCommand(CmdQuery, mc1.sqlConnection);
                                    //    cmd.ExecuteNonQuery();
                                    //    mc1.close();
                                    //}
                                    if (type == 1)
                        ...
                        //    mc1.close();
                        //    mc1.open();
                        //    SqlCommand cmd2 = new SqlCommand("INSERT INTO  i_facility.operationlog(OpMsg,OpDate,OpTime,OpDateTime,OpReason,MachineID) VALUES('" + @ex.ToString() + "','" + System.DateTime.Now.ToString("yyyy-MM-dd") + "','" + System.DateTime.Now.ToString("HH:mm:ss") + "','" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','Tool Stop',100" + mcid + ")", mc1.sqlConnection);
        ...
        private void insertMsgHisDB(ushort h, int machineid, DataTable dtOp)
        {
            ...
            for (int i = 0; i < dtOp.Rows.Count; i++)
            {
                ...
                {
                    //DataTable dt = new DataTable();
                    //String query = "SELECT Meassage From  i_facility.message_history_master WHERE MessageDateTime = '" + opmsgdatetime1 + "' AND MachineID = " + machineid + "";
                    ...
                    //    mc.close();
                    //}
                    List<message_history_master> msgs = new List<message_history_master>();
                    using (i_facility_shaktiEntities db1 = new i_facility_shaktiEntities())
                    {
                        msgs = db.message_history_master.Where(m => m.MessageDateTime.Equals(opmsgdatetime1) && m.MachineID == machineid).ToList();
                    }

                    if (msgs.Count == 0)
                    {
                        //DataTable dtshift = new DataTable();
                        //String querymsgtyp = "SELECT MessageType,MessageCode From  i_facility.message_code_master WHERE MessageCode = '" + opmsgcode1 + "'";
                        List<message_code_master> msgs1 = new List<message_code_master>();
                        using (i_facility_shaktiEntities db1 = new i_facility_shaktiEntities())
                        {
                            msgs1 = db.message_code_master.Where(m => m.MessageCode == opmsgcode1).ToList();
                        }
                        if (opmsgcode1.Length == 6)
                        {
                            //querymsgtyp = "SELECT MessageType,MessageCode From  i_facility.message_code_master WHERE MessageMCode = 'M" + opmsgcode1 + "'";
                            using (i_facility_shaktiEntities db1 = new i_facility_shaktiEntities())
                            {
                                msgs1 = db.message_code_master.Where(m => m.MessageCode == "M" + opmsgcode1).ToList();
                            }
                        }
                        //using (MsqlConnection mc = new MsqlConnection())
                        //{
                        ...
                        //    mc.close();
                        //}
                        //String querymsgtyp = "SELECT MessageType From message_code_master WHERE MessageCode = '" + opmsgcode1 + "'";

                        String msgtype = "";// dtmt.Rows[0][0].ToString();
                        opmsgcode1 = "";// dtmt.Rows[0][1].ToString();
                        String shift = null;
                        //String queryshift = "SELECT ShiftName,StartTime,EndTime FROM i_facility.shift_master WHERE IsDeleted = 0";
                        ...
                        //    mc.close();
                        //}
                        List<shift_master> msgs2 = new List<shift_master>();
                        using (i_facility_shaktiEntities db1 = new i_facility_shaktiEntities())
                        {
                            msgs2 = db.shift_master.Where(m => m.IsDeleted == 0).ToList();
                        }
                        ...
                        //{
                        //    mc.open();
                        //    SqlCommand cmd1 = new SqlCommand("INSERT INTO  i_facility.message_history_master(Meassage,MessageDate,MessageTime,MessageDateTime,MessageNo,InsertedOn,MachineID,MessageCode,MessageType,MessageShift,CorrectedDate,IsProgLock)" + //
                        ...
                        //    mc.close();
                        //}
                        message_history_master obj = new message_history_master();
                        ...
                        using (i_facility_shaktiEntities db1 = new i_facility_shaktiEntities())
                        {
                            db.message_history_master.Add(obj);
                            db.SaveChanges();
                        }

                        if (opmsgcode1 == "8006")
                        {
                            //String MsgDbquery = "SELECT MessageID,MessageDateTime From message_history_master WHERE MessageCode = '8006' AND IsProgLock = 0 AND MachineID = " + machineid + " Order By MessageID DESC;";
        ...
        private void insertAlarmHIsDB(ushort h, int machineid, DataTable dtAlrm)
        {
            ...
            IntoFile("dtAlrm:" + dtAlrm.Rows.Count);
            for (int i = 0; i < dtAlrm.Rows.Count; i++)
            {
                ...
                if (alamsgdate1 != null)
                {
                    ...
                    //{
                    //    mc.open();
                    //    String query = "SELECT AlarmNo From  i_facility.alarm_history_master WHERE AlarmDateTime = '" + alamsgdatetime1 + "' AND MachineID = " + machineid + " AND Axis_No = '" + alamsgaxisno1 + "'";
                    //    SqlDataAdapter da = new SqlDataAdapter(query, mc.msqlConnection);
                    //    da.Fill(dt);
                    //    mc.close();
                    //}

                    DateTime alarmdatetime = Convert.ToDateTime(alamsgdatetime1);
                    IntoFile("alarmDatetime" + alarmdatetime);
                    List<alarm_history_master> alarmhis = new List<alarm_history_master>();

                    using (i_facility_shaktiEntities db1 = new i_facility_shaktiEntities())
                    {
                        alarmhis = db1.alarm_history_master.Where(m => m.AlarmDateTime == alarmdatetime && m.MachineID == machineid && m.Axis_No == alamsgaxisno1).ToList();
                    }
                    IntoFile("alarmhis" + alarmhis.Count);
                    if (alarmhis.Count == 0)
                    {
                        //if (alamgrp1 == "6" || alamgrp1 == "9" || alamgrp1 == "0" || alamgrp1 == "1" || alamgrp1 == "5")
                        {
                            ...
                            IntoFile("alamsgdate1" + alamsgdate1);
                            ...
                            else if ((msgstime >= s3t1 && msgstime <= s3t4) || (msgstime >= s3t3 && msgstime < s3t2))
                            {
                                ...
                                if (msgstime >= s3t3 && msgstime < s3t2)
                                {
                                    CorrectedDate = Convert.ToDateTime(CorrectedDate).AddDays(-1).ToString("yyyy-MM-dd");
                                }
                            }
                            //using (MsqlConnection mc = new MsqlConnection())
                            //{
                            //    mc.open();
                            //    SqlCommand cmd1 = new SqlCommand("INSERT INTO  i_facility.alarm_history_master(AlarmMessage,AlarmDateTime,AlarmNo,Axis_No,Axis_Num,Abs_Pos,InsertedOn,MachineID,Shift,CorrectedDate)" +
                            ...
                            //    mc.close();
                            //}
                            alarm_history_master obj = new alarm_history_master();
                            ...
                            using (i_facility_shaktiEntities db1 = new i_facility_shaktiEntities())
                            {
                                db1.alarm_history_master.Add(obj);
                                db1.SaveChanges();

                                IntoFile("ALarm inserted");
                            }
                        }
                    }
                }
        ...
        private void insertAlarmHIsDB2(ushort h, int machineid, DataTable dtAlrm)
        {
            ...
            for (int i = 0; i < dtAlrm.Rows.Count; i++)
            {
                ...
                if (alamsgdate1 != null)
                {
                    ...
                    //{
                    //    mc.open();
                    //    String query = "SELECT AlarmNo From  alarm_history_master WHERE AlarmDateTime = '" + alamsgdatetime1 + "' AND MachineID = " + machineid + " AND Axis_No = '" + alamsgaxisno1 + "' AND AlarmNo = '" + alamsgalmno1 + "'";
                    //    SqlDataAdapter da = new SqlDataAdapter(query, mc.msqlConnection);
                    //    da.Fill(dt);
                    //    mc.close();
                    //}
                    List<alarm_history_master> alarm = new List<alarm_history_master>();
                    DateTime alaramdatetime = Convert.ToDateTime(alamsgdatetime1);

                    using (i_facility_shaktiEntities db1 = new i_facility_shaktiEntities())
                    {
                        alarm = db1.alarm_history_master.Where(m => m.AlarmDateTime == alaramdatetime && m.MachineID == machineid && m.Axis_No == alamsgaxisno1 && m.AlarmNo == Convert.ToString(alamsgalmno1)).ToList();
                    }
                    IntoFile("alarm Count:" + alarm.Count);
                    if (alarm.Count == 0)
                    {
                        ...
                        IntoFile("Alarm CorrectedDate" + CorrectedDate);
                        if (msgstime >= s1t1 && msgstime < s1t2)
                        {
                            shift = "1";
                        }
                        ...
                        //{
                        //    mc.open();
                        //    SqlCommand cmd1 = new SqlCommand("INSERT INTO  alarm_history_master(AlarmMessage,AlarmDateTime,AlarmNo,Axis_No,Axis_Num,Abs_Pos,InsertedOn,MachineID,Shift,CorrectedDate)" +
        ...
        private int programfilter(ushort h, int machineid, String MessageDateTime) //, int MessageDBRow
        {
            MsqlConnection mc = new MsqlConnection();
            mc.open();
            String GetMachineDispName = "SELECT MachineDisplayName FROM  tblmachinedetails WHERE MachineID = " + machineid + ";";
            SqlDataAdapter daTC = new SqlDataAdapter(GetMachineDispName, mc.msqlConnection);
            System.Data.DataTable dtTC = new System.Data.DataTable();
            daTC.Fill(dtTC);
            bool SendMail = false;

            String queryProgNo = "SELECT ProgramNum,ProgDBit From  tblmachinedetails WHERE IsDeleted = 0 AND MachineID = " + machineid + " order by MachineID";
            SqlDataAdapter daProgNo = new SqlDataAdapter(queryProgNo, mc.msqlConnection);
                ...
                if (opti1 == 0)
                {
                    ...
                    if (progret1 == 0)
                    {
                        ...
                        if (emp == 0)
                        {
                            ...
                            {
                                SA.open();
                                SqlCommand cmd = new SqlCommand("INSERT INTO  i_facility.operationlog(OpMsg,OpDate,OpTime,OpDateTime,OpReason,MachineID) VALUES('EMP CODE KeyWord Wrong.','" + System.DateTime.Now.ToString("yyyy-MM-dd") + "','" + System.DateTime.Now.ToString("HH:mm:ss") + "','" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','EMP CODE'," + machineid + ")", SA.msqlConnection);
                                cmd.ExecuteNonQuery();
                                SA.close();
                            }
                            SendMail = true;
                        ...
                        if (shf == 0)
                        {
                            ...
                            {
                                SA.open();
                                SqlCommand cmd = new SqlCommand("INSERT INTO  i_facility.operationlog(OpMsg,OpDate,OpTime,OpDateTime,OpReason,MachineID) VALUES('SHIFT KeyWord Wrong.','" + System.DateTime.Now.ToString("yyyy-MM-dd") + "','" + System.DateTime.Now.ToString("HH:mm:ss") + "','" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','SHIFT'," + machineid + ")", mc.msqlConnection);
                                cmd.ExecuteNonQuery();
                                SA.close();
                            }
                            SendMail = true;
                        ...
                        if (op1 == 0)
                        {
                            ...
                            {
                                SA.open();
                                SqlCommand cmd = new SqlCommand("INSERT INTO  i_facility.operationlog(OpMsg,OpDate,OpTime,OpDateTime,OpReason,MachineID) VALUES('OPN 1 KeyWord Wrong.','" + System.DateTime.Now.ToString("yyyy-MM-dd") + "','" + System.DateTime.Now.ToString("HH:mm:ss") + "','" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','OPN 1'," + machineid + ")", SA.msqlConnection);
                                cmd.ExecuteNonQuery();
                                SA.close();
                            }
                            SendMail = true;
                        ...
                        if (op2 == 0)
                        {
                            ...
                            {
                                SA.open();
                                SqlCommand cmd = new SqlCommand("INSERT INTO  i_facility.operationlog(OpMsg,OpDate,OpTime,OpDateTime,OpReason,MachineID) VALUES('OPN 2 KeyWord Wrong.','" + System.DateTime.Now.ToString("yyyy-MM-dd") + "','" + System.DateTime.Now.ToString("HH:mm:ss") + "','" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','OPN 2'," + machineid + ")", SA.msqlConnection);
                                cmd.ExecuteNonQuery();
                                SA.close();
                            }
                            SendMail = true;
                        ...
                        if (op3 == 0)
                        {
                            mc.close();
                            mc.open();
                            SqlCommand cmd = new SqlCommand("INSERT INTO  i_facility.operationlog(OpMsg,OpDate,OpTime,OpDateTime,OpReason,MachineID) VALUES('OPN 3 KeyWord Wrong.','" + System.DateTime.Now.ToString("yyyy-MM-dd") + "','" + System.DateTime.Now.ToString("HH:mm:ss") + "','" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','OPN 3'," + machineid + ")", mc.msqlConnection);
                            cmd.ExecuteNonQuery();
                            mc.close();
                            SendMail = true;
                        }
                        if (wo1 == 0)
                        {
                            mc.close();
                            mc.open();
                            SqlCommand cmd = new SqlCommand("INSERT INTO  i_facility.operationlog(OpMsg,OpDate,OpTime,OpDateTime,OpReason,MachineID) VALUES('WO 1 KeyWord Wrong.','" + System.DateTime.Now.ToString("yyyy-MM-dd") + "','" + System.DateTime.Now.ToString("HH:mm:ss") + "','" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','Emp Code'," + machineid + ")", mc.msqlConnection);
                            cmd.ExecuteNonQuery();
                            mc.close();
                            SendMail = true;
                        }
                        if (wo2 == 0)
                        {
                            mc.close();
                            mc.open();
                            SqlCommand cmd = new SqlCommand("INSERT INTO  i_facility.operationlog(OpMsg,OpDate,OpTime,OpDateTime,OpReason,MachineID) VALUES('WO 2 KeyWord Wrong.','" + System.DateTime.Now.ToString("yyyy-MM-dd") + "','" + System.DateTime.Now.ToString("HH:mm:ss") + "','" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','WO 2'," + machineid + ")", mc.msqlConnection);
                            cmd.ExecuteNonQuery();
                            mc.close();
                            SendMail = true;
                        }
                        if (wo3 == 0)
                        {
                            mc.close();
                            mc.open();
                            SqlCommand cmd = new SqlCommand("INSERT INTO  i_facility.operationlog(OpMsg,OpDate,OpTime,OpDateTime,OpReason,MachineID) VALUES('WO 3 KeyWord Wrong.','" + System.DateTime.Now.ToString("yyyy-MM-dd") + "','" + System.DateTime.Now.ToString("HH:mm:ss") + "','" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','WO 3'," + machineid + ")", mc.msqlConnection);
                            cmd.ExecuteNonQuery();
                            mc.close();
                            SendMail = true;
                        }
                        if (pp1 == 0)
                        {
                            mc.close();
                            mc.open();
                            SqlCommand cmd = new SqlCommand("INSERT INTO  i_facility.operationlog(OpMsg,OpDate,OpTime,OpDateTime,OpReason,MachineID) VALUES('PART PRO 1 KeyWord Wrong.','" + System.DateTime.Now.ToString("yyyy-MM-dd") + "','" + System.DateTime.Now.ToString("HH:mm:ss") + "','" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','PART PRO 1'," + machineid + ")", mc.msqlConnection);
                            cmd.ExecuteNonQuery();
                            mc.close();
                            SendMail = true;
                        }
                        if (pp2 == 0)
                        {
                            mc.close();
                            mc.open();
                            SqlCommand cmd = new SqlCommand("INSERT INTO  i_facility.operationlog(OpMsg,OpDate,OpTime,OpDateTime,OpReason,MachineID) VALUES('PART PRO 2 KeyWord Wrong.','" + System.DateTime.Now.ToString("yyyy-MM-dd") + "','" + System.DateTime.Now.ToString("HH:mm:ss") + "','" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','PART PRO 2'," + machineid + ")", mc.msqlConnection);
                            cmd.ExecuteNonQuery();
                            mc.close();
                            SendMail = true;
                        }
                        if (pp3 == 0)
                        {
                            mc.close();
                            mc.open();
                            SqlCommand cmd = new SqlCommand("INSERT INTO  i_facility.operationlog(OpMsg,OpDate,OpTime,OpDateTime,OpReason,MachineID) VALUES('PART PRO 3 KeyWord Wrong.','" + System.DateTime.Now.ToString("yyyy-MM-dd") + "','" + System.DateTime.Now.ToString("HH:mm:ss") + "','" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','PART PRO 3'," + machineid + ")", mc.msqlConnection);
                            cmd.ExecuteNonQuery();
                            mc.close();
                            SendMail = true;
                        }
                        if (pa1 == 0)
                        {
                            mc.close();
                            mc.open();
                            SqlCommand cmd = new SqlCommand("INSERT INTO  i_facility.operationlog(OpMsg,OpDate,OpTime,OpDateTime,OpReason,MachineID) VALUES('PART ACC 1 KeyWord Wrong.','" + System.DateTime.Now.ToString("yyyy-MM-dd") + "','" + System.DateTime.Now.ToString("HH:mm:ss") + "','" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','PART ACC 1'," + machineid + ")", mc.msqlConnection);
                            cmd.ExecuteNonQuery();
                            mc.close();
                            SendMail = true;
                        }
                        if (pa2 == 0)
                        {
                            mc.close();
                            mc.open();
                            SqlCommand cmd = new SqlCommand("INSERT INTO  i_facility.operationlog(OpMsg,OpDate,OpTime,OpDateTime,OpReason,MachineID) VALUES('PART ACC 2 KeyWord Wrong.','" + System.DateTime.Now.ToString("yyyy-MM-dd") + "','" + System.DateTime.Now.ToString("HH:mm:ss") + "','" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','PART ACC 2'," + machineid + ")", mc.msqlConnection);
                            cmd.ExecuteNonQuery();
                            mc.close();
                            SendMail = true;
                        }
                        if (pa3 == 0)
                        {
                            mc.close();
                            mc.open();
                            SqlCommand cmd = new SqlCommand("INSERT INTO  i_facility.operationlog(OpMsg,OpDate,OpTime,OpDateTime,OpReason,MachineID) VALUES('PART ACC 3 KeyWord Wrong.','" + System.DateTime.Now.ToString("yyyy-MM-dd") + "','" + System.DateTime.Now.ToString("HH:mm:ss") + "','" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','PART ACC 3'," + machineid + ")", mc.msqlConnection);
                            cmd.ExecuteNonQuery();
                            mc.close();
                            SendMail = true;
                        }
                        ...
                        if (!SendMail)
                        {
                            //SqlCommand cmd = new SqlCommand("INSERT INTO program_temp(ProgramData,ProgramDateTime,MachineID) VALUES('" + sb.ToString() + "','" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "'," + machineid + ")", mc.sqlConnection);
                            //cmd.ExecuteNonQuery();

                            SqlCommand cmd1 = new SqlCommand("INSERT INTO  i_facility.program_master(EmployeeCode,WorkOrderNo1,PartsProduced1,PartsAccepted1,PartsRejected1,MachineID,InsertedOn,ProgramDate,ProgramTime,ProgramDateTime,OpnCode1,OpnCode2,WorkOrderNo2,PartsProduced2,PartsAccepted2,PartsRejected2,OpnCode3,WorkOrderNo3,PartsProduced3,PartsAccepted3,PartsRejected3,Shift,CreatedOn)" +
                                "VALUES('" + EmpCode + "','" + WorkOrder1 + "'," + PartsPro1 + "," + PartsAccep1 + "," + PartsRej1 + "," + machineid + ",'" + Convert.ToDateTime(MessageDateTime).ToString("yyyy-MM-dd HH:mm:ss") + "','" + Convert.ToDateTime(MessageDateTime).ToString("yyyy-MM-dd") + "','" + Convert.ToDateTime(MessageDateTime).ToString("HH:mm:ss") + "','" + Convert.ToDateTime(MessageDateTime).ToString("yyyy-MM-dd HH:mm:ss") + "','" + OpnCode1 + "','" + OpnCode2 + "','" + WorkOrder2 + "'," + PartsPro2 + "," + PartsAccep2 + "," + PartsRej2 + ",'" + OpnCode3 + "','" + WorkOrder3 + "'," + PartsPro3 + "," + PartsAccep3 + "," + PartsRej3 + ",'" + Shift + "','" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "')", mc.msqlConnection);
                            cmd1.ExecuteNonQuery();

                            SqlCommand cmd2 = new SqlCommand("Update  i_facility.message_history_master SET IsProgLock = 1 WHERE MachineID = " + machineid + " ", mc.msqlConnection);
                            cmd2.ExecuteNonQuery();
        ...
        private int programfilter1(ushort h, int machineid)
        {
            ...
            daTC.Fill(dtTC);
            bool SendMail = false;
            String queryProgNo = "SELECT ProgramNum From  tblmachinedetails WHERE IsDeleted = 0 AND MachineID = " + machineid + " order by MachineID";
            SqlDataAdapter daProgNo = new SqlDataAdapter(queryProgNo, mc.msqlConnection);
            DataTable dtProgNo = new DataTable();
            daProgNo.Fill(dtProgNo);
            int progret, opti1, progret1;
                ...
                if (opti1 == 0)
                {
                    ...
                    if (progret1 == 0)
                    {
                        ...
                        if (emp == 0)
                        {
                            mc.close();
                            mc.open();
                            SqlCommand cmd = new SqlCommand("INSERT INTO  i_facility.operationlog(OpMsg,OpDate,OpTime,OpDateTime,OpReason,MachineID) VALUES('EMP CODE KeyWord Wrong.','" + System.DateTime.Now.ToString("yyyy-MM-dd") + "','" + System.DateTime.Now.ToString("HH:mm:ss") + "','" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','EMP CODE'," + machineid + ")", mc.msqlConnection);
                            cmd.ExecuteNonQuery();
                            mc.close();
                            SendMail = true;
                        }
                        ...
                        if (op1 == 0)
                        {
                            mc.close();
                            mc.open();
                            SqlCommand cmd = new SqlCommand("INSERT INTO  i_facility.operationlog(OpMsg,OpDate,OpTime,OpDateTime,OpReason,MachineID) VALUES('OPN 1 KeyWord Wrong.','" + System.DateTime.Now.ToString("yyyy-MM-dd") + "','" + System.DateTime.Now.ToString("HH:mm:ss") + "','" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','OPN 1'," + machineid + ")", mc.msqlConnection);
                            cmd.ExecuteNonQuery();
                            mc.close();
                            SendMail = true;
                        }
                        if (op2 == 0)
                        {
                            mc.close();
                            mc.open();
                            SqlCommand cmd = new SqlCommand("INSERT INTO  i_facility.operationlog(OpMsg,OpDate,OpTime,OpDateTime,OpReason,MachineID) VALUES('OPN 2 KeyWord Wrong.','" + System.DateTime.Now.ToString("yyyy-MM-dd") + "','" + System.DateTime.Now.ToString("HH:mm:ss") + "','" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','OPN 2'," + machineid + ")", mc.msqlConnection);
                            cmd.ExecuteNonQuery();
                            mc.close();
                            SendMail = true;
                        }
                        ...
                        if (dt.Rows.Count == 0 && !SendMail)
                        {
                            ...
                            SqlCommand cmd1 = new SqlCommand("INSERT INTO  program_master(EmployeeCode,WorkOrderNo1,PartsProduced1,PartsAccepted1,PartsRejected1,MachineID,InsertedOn,ProgramDate,ProgramTime,ProgramDateTime,OpnCode1,OpnCode2,WorkOrderNo2,PartsProduced2,PartsAccepted2,PartsRejected2,OpnCode3,WorkOrderNo3,PartsProduced3,PartsAccepted3,PartsRejected3,Shift)" +
                                "VALUES('" + EmpCode + "','" + WorkOrder1 + "'," + PartsPro1 + "," + PartsAccep1 + "," + PartsRej1 + "," + machineid + ",'" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','" + System.DateTime.Now.ToString("yyyy-MM-dd") + "','" + System.DateTime.Now.ToString("HH:mm:ss") + "','" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','" + OpnCode1 + "','" + OpnCode2 + "','" + WorkOrder2 + "'," + PartsPro2 + "," + PartsAccep2 + "," + PartsRej2 + ",'" + OpnCode3 + "','" + WorkOrder3 + "'," + PartsPro3 + "," + PartsAccep3 + "," + PartsRej3 + ",'" + Shift + "')", mc.msqlConnection);
                            cmd1.ExecuteNonQuery();
                            mc.close();
                        }
                        if (SendMail)
        ...
        private void ProgEmailEscalation(String MailBody, String MachineInv, int machineid)
        {
            MsqlConnection mc = new MsqlConnection();
            mc.open();
            string SQLFromAdd = "SELECT FromEmailAdd,Password,username,domain FROM  frommail Where IsDeleted = 0";
            SqlDataAdapter daTC = new SqlDataAdapter(SQLFromAdd, mc.msqlConnection);
            System.Data.DataTable dtTC = new System.Data.DataTable();
            daTC.Fill(dtTC);

            string SQLToAdd = "SELECT EmailID FROM  MailMasterProgEsc Where TOAdd = 1 AND IsDeleted = 0";
            SqlDataAdapter daTO = new SqlDataAdapter(SQLToAdd, mc.msqlConnection);
            ...
            SqlDataAdapter daCC = new SqlDataAdapter(SQLCCAdd, mc.msqlConnection);
            ...
            SqlDataAdapter daBCC = new SqlDataAdapter(SQLBCCAdd, mc.msqlConnection);
            ...
            {
                SqlCommand cmd = new SqlCommand("INSERT INTO  i_facility.operationlog(OpMsg,OpDate,OpTime,OpDateTime,OpReason,MachineID) VALUES('Program Escalation Email was not Sent: " + ex.Message + "','" + System.DateTime.Now.ToString("yyyy-MM-dd") + "','" + System.DateTime.Now.ToString("HH:mm:ss") + "','" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','App Error'," + machineid + ")", mc.msqlConnection);
                cmd.ExecuteNonQuery();
            }
            mc.close();
        }
        ...
        private void ProgramInsertCheck(ushort h, int machineid)
        {
            MsqlConnection mc = new MsqlConnection();
            mc.open();
            String query = "SELECT MessageID,MessageDateTime From message_history_master WHERE MessageCode = '8006' AND IsProgLock = 0 AND MachineID = " + machineid + " Order By MachineID DESC;";
            SqlDataAdapter da = new SqlDataAdapter(query, mc.msqlConnection);
            DataTable dt = new DataTable();
            da.Fill(dt);
            mc.close();
            if (dt.Rows.Count != 0)
            {
                ...
                if (ret == 4)
                {
                    mc.close();
                    mc.open();
                    SqlCommand cmd = new SqlCommand("INSERT INTO i_facility.operationlog(OpMsg,OpDate,OpTime,OpDateTime,OpReason,MachineID) VALUES('NC Program Cant be read, due to upload error from M/c.','" + System.DateTime.Now.ToString("yyyy-MM-dd") + "','" + System.DateTime.Now.ToString("HH:mm:ss") + "','" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','M/c Error'," + machineid + ")", mc.msqlConnection);
                    cmd.ExecuteNonQuery();
                    mc.close();
                }
            }
        ...
        private void InsertAxisDetails(ushort h, int machineid, int NoOfAxis)
        {
            ...
            for (int i = 0; i < NoOfAxis; i++)
            {
                ...
                using (MsqlConnection mc = new MsqlConnection())
                {
                    ...
                    {
                        SqlCommand cmd1 = new SqlCommand("INSERT INTO tbl_axisdetails1(MachineID,Axis,AbsPos,RelPos,MacPos,DistPos,StartTime,IsDeleted,InsertedOn)" + //
                            "VALUES('" + machineid + "','" + AxisNo + "','" + AbsPos + "'," + RelPos + ",'" + MacPos + "','" + DisPos + "','" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:00") + "',0,'" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "')", mc.msqlConnection);
                        cmd1.ExecuteNonQuery();
                        insert++;
                    }
                    ...
                    if (i == 0 || insert == 0)
                    {
                        ...
                        SqlCommand cmd2 = new SqlCommand("INSERT INTO  tbl_axisdetails2(MachineID,FeedRate,SpindleLoad,SpindleSpeed,StartTime,IsDeleted,InsertedOn)" + //
                            "VALUES('" + machineid + "','" + FeedRate + "','" + SpiLoadMain.ToString() + "'," + SpindleSpeed + ",'" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:00") + "',0,'" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "')", mc.msqlConnection);
                        cmd2.ExecuteNonQuery();
                    }
                    mc.close();
                }
                ...
                switch (i)
                {
                    case 0:
                        ...
                        using (MsqlConnection mc = new MsqlConnection())
                        {
                            ...
                            {
                                mc.open();
                                SqlCommand cmd1 = new SqlCommand("INSERT INTO  tbl_servodetails(MachineID,ServoAxis,ServoLoadMeter,LoadCurrent,LoadCurrentAmp,StartDateTime,IsDeleted,InsertedOn,InsertedBy)" + //
                                        "VALUES('" + machineid + "','" + AxisName + "','" + ServLoadMain + "','" + ServCurPerMain + "','" + ServcurrAmpMain + "','" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "',0,'" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "',1)", mc.msqlConnection);
                                cmd1.ExecuteNonQuery();
                                mc.close();
                            }
                        }
                    ...
                    case 1:
                        ...
                        using (MsqlConnection mc = new MsqlConnection())
                        {
                            if (AxisName2 != "" || AxisName2 != null)
                            {
                                mc.open();
                                SqlCommand cmd1 = new SqlCommand("INSERT INTO  tbl_servodetails(MachineID,ServoAxis,ServoLoadMeter,LoadCurrent,LoadCurrentAmp,StartDateTime,IsDeleted,InsertedOn,InsertedBy)" + //
                                        "VALUES('" + machineid + "','" + AxisName2 + "','" + ServLoadMain2 + "','" + ServCurPerMain2 + "','" + ServcurrAmpMain2 + "','" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "',0,'" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "',1)", mc.msqlConnection);
                                cmd1.ExecuteNonQuery();
                                mc.close();
                            }
                        }
                    ...
                    case 2:
                        ...
                        using (MsqlConnection mc = new MsqlConnection())
                        {
                            if (AxisName3 != "" || AxisName3 != null)
                            {
                                mc.open();
                                SqlCommand cmd1 = new SqlCommand("INSERT INTO  tbl_servodetails(MachineID,ServoAxis,ServoLoadMeter,LoadCurrent,LoadCurrentAmp,StartDateTime,IsDeleted,InsertedOn,InsertedBy)" + //
                                        "VALUES('" + machineid + "','" + AxisName3 + "','" + ServLoadMain3 + "','" + ServCurPerMain3 + "','" + ServcurrAmpMain3 + "','" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "',0,'" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "',1)", mc.msqlConnection);
                                cmd1.ExecuteNonQuery();
                                mc.close();
                            }
                        }
                    ...
                    case 3:
                        ...
                        using (MsqlConnection mc = new MsqlConnection())
                        {
                            if (AxisName4 != "" || AxisName4 != null)
                            {
                                mc.open();
                                SqlCommand cmd1 = new SqlCommand("INSERT INTO  tbl_servodetails(MachineID,ServoAxis,ServoLoadMeter,LoadCurrent,LoadCurrentAmp,StartDateTime,IsDeleted,InsertedOn,InsertedBy)" + //
                                        "VALUES('" + machineid + "','" + AxisName4 + "','" + ServLoadMain4 + "','" + ServCurPerMain4 + "','" + ServcurrAmpMain4 + "','" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "',0,'" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "',1)", mc.msqlConnection);
                                cmd1.ExecuteNonQuery();
                                mc.close();
                            }
                        }
        ...
        public bool IncrementATCCounter(ushort h, int MacID, int CurATCVal)
        {
            ...
            {
                mc.open();
                string SelectQuery = "SELECT HMIID FROM  tblworkorderentry where MachineID = '" + MacID + "' and IsStarted = 1 and IsFinished = 0 and IsHold = 0 order by ToolLifeCycID DESC ;";
            ...
            if (CycleStart == 1 && WOID != 0)
            {
                ...
                if (dataHolder.Rows.Count > 0)
                {
                    ...
                    {
                        mc.open();
                        SqlCommand cmdUpdateRows = new SqlCommand("UPDATE [unitworksccs].[unitworkccs].[tbltoollifeoperator] Set toollifecounter = " + CycCounter + ", IsCycleStart = 1  Where ToolLifeID = " + ToolLifeID + "");
            ...
            else if (CycleStart == 0 && WOID != 0)
            {
                ...
                {
                    mc.open();
                    SqlCommand cmdUpdateRows = new SqlCommand("UPDATE [unitworksccs].[unitworkccs].[tbltoollifeoperator] Set IsCycleStart = 0 Where HMIID = " + WOID + " and IsCompleted = 0;");
                    cmdUpdateRows.ExecuteNonQuery();
                    mc.close();
                }
                ...
                //        {
                //            mc.open();
                //            SqlCommand cmdUpdateRows = new SqlCommand("UPDATE  tblToolLifeCycle Set CycleCounter = " + PartCounter + ", CycleEndTime = '" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "', PartCounter =  Where ToolLifeCycID = " + ToolLifeID + "");
                //            //SqlCommand cmdUpdateRows = new SqlCommand("Insert into  tblToolLifeCycle (MachineID,ToolNo,CycleCounter,CycleStartTime,CorrectedDate)" + "" +
        ...
        #region NEW CODE            
        private void getmachinemode(int MacID, string IPAddress, ushort h, int MachineLockBit, int MachineIdleBit, int MachineUnlockBit, int MachineIdleMin, int ConnectionRet, int ConnectionRetErr, int EnableLock, shift_master shiftmasterdet, int parameterexcept)
        {
            try
            {
                DateTime correctedDate = DateTime.Now;
                tbldaytiming Daytimings = new tbldaytiming();
                using (i_facility_shaktiEntities db = new i_facility_shaktiEntities())
                {
                    Daytimings = db.tbldaytimings.Where(m => m.IsDeleted == 0).FirstOrDefault();
                }
                ...
                List<parameters_master> parameterdet = new List<parameters_master>();
                List<tbllivemode> ModeDetList = new List<tbllivemode>();
                tblmachinedetail Machinedet = new tblmachinedetail();
                using (i_facility_shaktiEntities db = new i_facility_shaktiEntities())
                {
                    parameterdet = db.parameters_master.Where(m => m.MachineID == MacID).OrderByDescending(m => m.ParameterID).ToList();
                    ModeDetList = db.tbllivemodes.Where(m => m.MachineID == MacID && m.IsCompleted == 0 || (m.IsCompleted == 1 && m.ModeTypeEnd == 0)).OrderByDescending(m => m.ModeID).ToList();
                    Machinedet = db.tblmachinedetails.Find(MacID);
                }
                ...
                IntoFile("ModeCount:" + ModeDetList.Count + " MachineID" + MacID);
                if (ModeDetList.Count == 1)
                {
                        ...
                        if (shiftmasterdet != null)
                        {
                            try
                            {
                                ...
                                if (ShiftETDT.Hour <= nowdate.Hour)
                                {
                                    ...
                                    IntoFile("BeforeShiftend:" + MacID);
                                    UpdatetbllivemodeDetails(NowDateCalc, durationinsec, ModeID);
                                    IntoFile("afterShiftend:" + MacID);
                                    ...
                                    if (parameterexcept == 1) // For isparameter as 1 && cutting time based logic 
                                    {
                                        Temp_DeleteModeDetails(MacID);  //for the same machine if there is a idle mode at  the temptable then it has to be deleted
                                    }
                                }
                            }
                            catch (Exception ex)
                    ...
                    else if (MacMode != "MNT")
                    {
                        try
                        {
                            List<tbllivemode> ModeDetList1 = new List<tbllivemode>();
                            using (i_facility_shaktiEntities db = new i_facility_shaktiEntities())
                            {
                                ModeDetList1 = db.tbllivemodes.Where(m => m.MachineID == MacID && m.IsCompleted == 0 || (m.IsCompleted == 1 && m.ModeTypeEnd == 0)).ToList();
                            }
                            ...
                            ModeType = ModeDetList1[0].ModeType.ToString();
                            int pingCounter = 0;
                            TryPing:
                            //Ping p = new Ping();
                            //PingReply r;
                            ...
                            if (ConnectionRet == 0)
                            {
                                ...
                                {
                                    int diff = 0;
                                    if (parameterexcept == 1)  // for cutting time based machines
                                    {
                                        int prvoptime = Convert.ToInt32(parameterdet[1].CuttingTime);
                                        int proptime = Convert.ToInt32(parameterdet[0].CuttingTime);
                                        diff = proptime - prvoptime;
                                    }
                                    else   // For Operating time based machines
                                    {
                                    ...
                                    else
                                    {
                                        if (MacMode != "IDLE")
                                        {
                                            if (parameterexcept == 1)  // For isparameter as 1 && cutting time based logic 
                                            {
                                                ...
                                                Temp_INSERTMODE("IDLE", MacID, CorrectedDatestring, nowdate, "IDLE");  // initially pushing IDLE Mode to Temptblmode
                                                Temp_tblmode objtempmode = new Temp_tblmode();
                                                using (i_facility_shaktiEntities db = new i_facility_shaktiEntities())
                                                {
                                                    objtempmode = db.Temp_tblmode.Where(m => m.MachineID == MacID && m.isdeleted == 0 && m.CorrectedDate == CorrectedDatestring).OrderByDescending(m => m.tempModeID).FirstOrDefault();
                                                }
                                                if (objtempmode != null)
                                                {
                                                    DateTime tempstarttime = Convert.ToDateTime(objtempmode.startTime);
                                                    int durationinsec = Convert.ToInt32(DateTime.Now.Subtract(Convert.ToDateTime(tempstarttime)).TotalSeconds);
                                                    if (durationinsec > 120)
                                                    {
                                                        int durationinsec1 = Convert.ToInt32(tempstarttime.Subtract(Convert.ToDateTime(StartTime)).TotalSeconds);
                                                        UpdatetbllivemodeDetails(tempstarttime, durationinsec1, 0, ModeID);
                                                        INSERTMODE("IDLE", MacID, 1, System.DateTime.Now, correctedDate, tempstarttime, "YELLOW", "IDLE", shiftid);
                                                        Temp_DeleteModeDetails(MacID);  //for the same machine if there is a idle mode at  the temptable then it has to be deleted
                                                    }
                                                }
                                            }
                                            else if (parameterexcept != 1) // For all Parameters
                                            {
                                                int durationinsec1 = Convert.ToInt32(nowdate.Subtract(Convert.ToDateTime(StartTime)).TotalSeconds);
                                                UpdatetbllivemodeDetails(nowdate, durationinsec1, 0, ModeID);
                ...
                else if (ModeDetList.Count > 1)
                {
                    ...
                    using (i_facility_shaktiEntities db = new i_facility_shaktiEntities())
                    {
                        ...
                        dtMode1 = db.tbllivemodes.Where(m => m.IsCompleted == 0 && m.MachineID == MacID).OrderBy(m => m.ModeID).ToList();
                    }
                ...
                else
                {
                    try
                    {
                        tbllivemode ModedetPoff = new tbllivemode();
                        using (i_facility_shaktiEntities db = new i_facility_shaktiEntities())
                        {
                            ModedetPoff = db.tbllivemodes.Where(m => m.CorrectedDate == correctedDate && m.MachineID == MacID).OrderByDescending(m => m.ModeID).FirstOrDefault();
                        }
                        DateTime nowdate = DateTime.Now;
        ...
        private void getmachinemodeDayWise(int MacID, string IPAddress, ushort h, int MachineLockBit, int MachineIdleBit, int MachineUnlockBit, int MachineIdleMin, int ConnectionRet, int ConnectionRetErr, int EnableLock)
        {
            ...
            //{
            //    mc.open();
            //    String getparametersquery = "SELECT OperatingTime From  i_facility.parameters_master WHERE MachineID = " + MacID + " order by ParameterID DESC";
            ...
            if (ModeDetList.Count == 1)
            {
                ...
                if (correctedDate != CorrectedDateDB)
                {
                    ...
                    try
                    {
                        ...
                        //{
                        //    mc1.open();
                        //    SqlCommand cmdendprvmode = new SqlCommand("UPDATE i_facility.tbllivemode SET EndTime = '" + NowDateCalc.ToString("yyyy-MM-dd HH:mm:ss") + "', IsCompleted = 1, ModeTypeEnd = 1, DurationInSec = " + durationinsec + " Where ModeID = " + ModeID + "", mc1.msqlConnection);
                        ...
                        if (LossCodeID.Length == 0 && BDID.Length == 0)
                        {
                            ...
                            //{
                            //    sa.open();
                            //    SqlCommand cmdpoweroff = new SqlCommand("INSERT INTO i_facility.tbllivemode " +
                ...
                else if (MacMode != "MNT")
                {
                    try
                    {
                        ...
                        if (ConnectionRet == 0)
                        {
                            ...
                            //        parameterdet = db.parameters_master.Where(m => m.MachineID == MacID).ToList();
                            //    }
                            if (parameterdet.Count >= 2)
                            {
                                int prvoptime = Convert.ToInt32(parameterdet[1].OperatingTime);
                                ...
                                if (diff > 0)
                                {
                                    if (MacMode != "PROD")
                                    {
                                        ...
                                        //{
                                        //    sa.open();
                                        //    SqlCommand cmdendprvmode = new SqlCommand("UPDATE i_facility.tbllivemode SET" +
                                        ...
                                        ////    {
                                        ////        sa.open();
                                        ////        SqlCommand cmdendprvmode = new SqlCommand("UPDATE tbllivemode SET EndTime = '" + nowdate.ToString("yyyy-MM-dd HH:mm:ss") + "', IsCompleted = 1, ModeTypeEnd = 1, DurationInSec = " + durationinsec + " Where ModeID = " + ModeID + "", sa.msqlConnection);
                                        ////        int retend = cmdendprvmode.ExecuteNonQuery();
                                        ////        sa.close();
                                        ////    }
                                        ////}
                                ...
                                else
                                {
                                    if (MacMode != "IDLE")
                                    {
                                        ...
                                        //{
                                        //    sa.open();
                                        //    SqlCommand cmdendprvmode = new SqlCommand("UPDATE i_facility.tbllivemode SET EndTime = '" + nowdate.ToString("yyyy-MM-dd HH:mm:ss") + "', IsCompleted = 1, DurationInSec = " + durationinsec + ", ModeTypeEnd = 1, StartIdle = 0 Where ModeID = " + ModeID + "", sa.msqlConnection);
                                        //    int retend = cmdendprvmode.ExecuteNonQuery();
                                        //    sa.close();
                                        //    //setmachineUnlock(h, (ushort)MachineLockBit, (ushort)MachineUnlockBit);
                                        //}
                                        ...
                                        ////    {
                                        ////        sa.open();
                                        ////        SqlCommand cmdendprvmode = new SqlCommand("UPDATE tbllivemode SET EndTime = '" + nowdate.ToString("yyyy-MM-dd HH:mm:ss") + "', IsCompleted = 1, DurationInSec = " + durationinsec + ", ModeTypeEnd = 1 Where ModeID = " + ModeID + "", sa.msqlConnection);
                                        ////        int retend = cmdendprvmode.ExecuteNonQuery();
                                        ////        sa.close();
                                        ////        setmachineUnlock(h, (ushort)MachineLockBit, (ushort)MachineUnlockBit);
                                        ////    }
                                ...
                                if (MacMode == "IDLE" && StartIdle == 0)
                                {
                                    ...
                                    if (DurinMin >= MachineIdleMin)
                                    {
                                        ...
                                        //{
                                        //    mc1.open();
                                        //    SqlCommand cmdendprvmode = new SqlCommand("UPDATE i_facility.tbllivemode SET StartIdle = 1 Where ModeID = " + ModeID + "", mc1.msqlConnection);
                                        //    int retend = cmdendprvmode.ExecuteNonQuery();
                                        //    mc1.close();
                                        //    if (EnableLock == 1)
                                        //    {
            ...
            else if (dtMode.Rows.Count > 1)
            {
                ...
                for (int i = 0; i < (dtModeMultiple.Count - 1); i++)
                {
                    ...
                    {
                        //mc.open();
                        //SqlCommand cmdpoweroff = new SqlCommand("DELETE FROM tbllivemode where ModeID = " + Convert.ToInt32(dtModeMultiple.Rows[i][0]), mc.msqlConnection);
                        //int ret = cmdpoweroff.ExecuteNonQuery();
                        //mc.close();
                        DeleteModeDetails(dtModeMultiple[i].ModeID);
                    }
                ...
                for (int i = 0; i < (dtMode1.Count - 2); i++)
                {
                    ...
                    {
                        //mc.open();
                        //SqlCommand cmdpoweroff = new SqlCommand("DELETE FROM tbllivemode where ModeID = " + Convert.ToInt32(dtMode1.Rows[i][0]), mc.msqlConnection);
                        //int ret = cmdpoweroff.ExecuteNonQuery();
                        //mc.close();
                        DeleteModeDetails(dtMode1[i].ModeID);
                    }
            ...
            else
            {
                try
                {
                    ...
                    if (DtModePOFF.Rows.Count == 0)
                    {
                        ...
                        //{
                        //    mc1.open();
                        //    SqlCommand cmdpoweroff = new SqlCommand("INSERT INTO i_facility.tbllivemode (MacMode,MachineID,InsertedBy,InsertedOn,CorrectedDate,StartTime,ColorCode,ModeType) 
        ...
        //    {
        //        mc.open();
        //        String getparametersquery = "SELECT OperatingTime From  parameters_master WHERE MachineID = " + MacID + " order by ParameterID DESC";
        //        SqlDataAdapter da = new SqlDataAdapter(getparametersquery, mc.msqlConnection);
        //        DataTable dt = new DataTable();
        //        da.Fill(dt);
        ...
        //                SqlCommand cmdendprvmode = new SqlCommand("UPDATE tbllivemode SET EndTime = '" + nowdate.ToString("yyyy-MM-dd " + StartDateTime.AddSeconds(-1).TimeOfDay) + "', IsCompleted = 1, ModeTypeEnd = 1, DurationInSec = " + durationinsec + " Where ModeID = " + ModeID + "", mc.msqlConnection);
        //                int retend = cmdendprvmode.ExecuteNonQuery();
        //                if (LossCodeID.Length == 0 && BDID.Length == 0)
        //                {
        //                    SqlCommand cmdpoweroff = new SqlCommand("INSERT INTO tbllivemode (MacMode,MachineID,InsertedBy,InsertedOn,CorrectedDate,StartTime,ColorCode,ModeType) VALUES('" + MacMode + "'," + MacID + ",1,'" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','" + correctedDate + "','" + StartTimeNext + "','" + ColoCode + "','" + ModeType + "')", mc.msqlConnection);
        //                    int ret = cmdpoweroff.ExecuteNonQuery();
        //                }
        //                else if (LossCodeID.Length != 0)
        //                {
        //                    SqlCommand cmdpoweroff = new SqlCommand("INSERT INTO tbllivemode (MacMode,MachineID,InsertedBy,InsertedOn,CorrectedDate,StartTime,ColorCode, LossCodeID, ModeType) VALUES('" + MacMode + "'," + MacID + ",1,'" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','" + correctedDate + "','" + StartTimeNext + "','" + ColoCode + "'," + Convert.ToInt32(LossCodeID) + ",'" + ModeType + "'", mc.msqlConnection);
        //                    int ret = cmdpoweroff.ExecuteNonQuery();
        //                }
        //                else if (BDID.Length != 0)
        //                {
        //                    SqlCommand cmdpoweroff = new SqlCommand("INSERT INTO tbllivemode (MacMode,MachineID,InsertedBy,InsertedOn,CorrectedDate,StartTime,ColorCode, BreakdownID, ModeType) VALUES('" + MacMode + "'," + MacID + ",1,'" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','" + correctedDate + "','" + StartTimeNext + "','" + ColoCode + "'," + Convert.ToInt32(BDID) + ",'" + ModeType + "')", mc.msqlConnection);
        //                    int ret = cmdpoweroff.ExecuteNonQuery();
        //                }
        //                mc.close();
        //            }
        ...
        //                                if (ModeType == "IDLE" || ModeType == "POWEROFF")
        //                                {
        //                                    SqlCommand cmdendprvmode = new SqlCommand("UPDATE tbllivemode SET EndTime = '" + nowdate.ToString("yyyy-MM-dd HH:mm:ss") + "', IsCompleted = 1, DurationInSec = " + durationinsec + ", ModeTypeEnd = 1, StartIdle = 0 Where ModeID = " + ModeID + "", mc.msqlConnection);
        //                                    int retend = cmdendprvmode.ExecuteNonQuery();
        //                                }
        //                                else
        //                                {
        //                                    SqlCommand cmdendprvmode = new SqlCommand("UPDATE tbllivemode SET EndTime = '" + nowdate.ToString("yyyy-MM-dd HH:mm:ss") + "', IsCompleted = 1, ModeTypeEnd = 1, DurationInSec = " + durationinsec + " Where ModeID = " + ModeID + "", mc.msqlConnection);
        //                                    int retend = cmdendprvmode.ExecuteNonQuery();
        //                                }

        //                                SqlCommand cmdpoweroff = new SqlCommand("INSERT INTO tbllivemode (MacMode,MachineID,InsertedBy,InsertedOn,CorrectedDate,StartTime,ColorCode, ModeType) VALUES('PROD'," + MacID + ",1,'" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','" + correctedDate + "','" + nowdate.ToString("yyyy-MM-dd HH:mm:ss") + "','GREEN','PROD')", mc.msqlConnection);
        //                                int ret = cmdpoweroff.ExecuteNonQuery();
        //                                mc.close();
        //                            }
        //                        }
        ...
        //                                else
        //                                {
        //                                    SqlCommand cmdendprvmode = new SqlCommand("UPDATE tbllivemode SET EndTime = '" + nowdate.ToString("yyyy-MM-dd HH:mm:ss") + "', IsCompleted = 1, DurationInSec = " + durationinsec + ", ModeTypeEnd = 1 Where ModeID = " + ModeID + "", mc.msqlConnection);
        //                                    int retend = cmdendprvmode.ExecuteNonQuery();
        //                                    setmachineUnlock(h, (ushort)MachineLockBit, (ushort)MachineUnlockBit);
        //                                }

        //                                SqlCommand cmdpoweroff = new SqlCommand("INSERT INTO tbllivemode (MacMode,MachineID,InsertedBy,InsertedOn,CorrectedDate,StartTime,ColorCode, ModeType) VALUES('IDLE'," + MacID + ",1,'" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','" + correctedDate + "','" + nowdate.ToString("yyyy-MM-dd HH:mm:ss") + "','YELLOW','IDLE')", mc.msqlConnection);
        //                                int ret = cmdpoweroff.ExecuteNonQuery();
        //                                mc.close();
        //                            }
        //                        }
        ...
        //                            {
        //                                mc.open();
        //                                SqlCommand cmdendprvmode = new SqlCommand("UPDATE  tbllivemode SET StartIdle = 1 Where ModeID = " + ModeID + "", mc.msqlConnection);
        //                                int retend = cmdendprvmode.ExecuteNonQuery();
        //                                mc.close();
        //                                if (EnableLock == 1)
        //                                {
        ...
        //                        SqlCommand cmdpoweroff = new SqlCommand("INSERT INTO  tbllivemode (MacMode,MachineID,InsertedBy,InsertedOn,CorrectedDate,StartTime,ColorCode,ModeType) VALUES('POWEROFF'," + MacID + ",1,'" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','" + correctedDate + "','" + nowdate.ToString("yyyy-MM-dd HH:mm:ss") + "','BLUE','POWEROFF')", mc.msqlConnection);
        //                        int ret = cmdpoweroff.ExecuteNonQuery();
        //                        mc.close();
        //                    }
        //                    mc.close();
        ...
        private void updatemimics(int MacID)
        {
            ...
            {
                mc.open();
                String getmodequery = "SELECT MacMode From tbllivemode WHERE IsCompleted = 0 and MachineID = " + MacID + " order by ModeID DESC";
                SqlDataAdapter daMode = new SqlDataAdapter(getmodequery, mc.msqlConnection);
                daMode.Fill(dtMode);
                mc.close();
            }
            ...
            if (dtMode.Rows.Count > 0)
            {
                ...
                using (MsqlConnection mc = new MsqlConnection())
                {
                    using (SqlCommand cmd = new SqlCommand("SP_MinMod", mc.msqlConnection))
                    {
        ...
        private int CalculateCuttingTime(DateTime startDateTime, DateTime endDatetime, int machineId, int modeId)
        {
            ...
            try
            {
                ...
                using (i_facility_shaktiEntities db = new i_facility_shaktiEntities())
                {
                    parameters_master LastRow = db.parameters_master.Where(m => m.MachineID == machineId && m.InsertedOn >= startDateTime && m.InsertedOn <= endDatetime).OrderByDescending(m => m.ParameterID).FirstOrDefault();
                    parameters_master FirstRow = db.parameters_master.Where(m => m.MachineID == machineId && m.InsertedOn >= startDateTime && m.InsertedOn <= endDatetime).OrderBy(m => m.ParameterID).FirstOrDefault();
                ...
                using (i_facility_shaktiEntities db = new i_facility_shaktiEntities())
                {
                    tbllivemode updateRow = db.tbllivemodes.Find(modeId);
                    if (updateRow != null)
                    {
        ...
        private void DeleteModeDetails(int ModeID)
        {
            try
            {
                using (i_facility_shaktiEntities db = new i_facility_shaktiEntities())
                {
                    tbllivemode row = db.tbllivemodes.Find(ModeID);
        ...
        public void Temp_INSERTMODE(string MacMode, int MacID, string correctedDate, DateTime StartTimeNext, string ModeType)
        {
            try
            {
                ...
                Temp_tblmode tempmodedata = db.Temp_tblmode.Where(m => m.MachineID == MacID && m.isdeleted == 0 && m.CorrectedDate == correctedDate).OrderByDescending(m => m.tempModeID).FirstOrDefault();
                if (tempmodedata == null)
                {
                    Temp_tblmode row = new Temp_tblmode();
                    row.MachineID = MacID;
                    row.CorrectedDate = correctedDate;
                    row.startTime = StartTimeNext;
                    row.ModeType = ModeType;
        ...
        private void Temp_DeleteModeDetails(int MacID)
        {
            try
            {
                Temp_tblmode objtempmode = new Temp_tblmode();
                using (i_facility_shaktiEntities db = new i_facility_shaktiEntities())
                {
                    objtempmode = db.Temp_tblmode.Where(m => m.MachineID == MacID && m.isdeleted == 0 && m.ModeType == "IDLE").OrderByDescending(m => m.tempModeID).FirstOrDefault();
                }
                if (objtempmode != null)
                {
                    objtempmode.isdeleted = 1;
                    using (i_facility_shaktiEntities db = new i_facility_shaktiEntities())
                    {
                        db.Entry(objtempmode).State = System.Data.Entity.EntityState.Modified;
                        db.SaveChanges();
                        IntoFile("Delete tempmode details");
                    }
                }
            }
            catch (Exception ex)
        ...
        private string GetPartCount(ushort h)
        {
            string PartCount = "0";
            try
            {
                //Part Counter -- D Bit 8006, 8012 P1 & 8014 P2
                Focas1.IODBPMC0 rdpmcdataPartCounter = new Focas1.IODBPMC0();
                short adr_type = 9;
                short data_type = 0;
                ushort s_number = 8006;
                ushort e_number = 8006;
                ushort length = 9;
                short rdretLock = Focas1.pmc_rdpmcrng(h, adr_type, data_type, s_number, e_number, length, rdpmcdataPartCounter);
                //Machine LOCK D Bit
                Focas1.IODBPMC0 wrpmcdata = rdpmcdataPartCounter;
                string PartCounter = wrpmcdata.cdata[0].ToString();

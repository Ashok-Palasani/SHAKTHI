...
//        public DALBatch(i_facility_tsalContext _db, IConfiguration _config, IOptions<AppSettings> _appSettings)
//        {
//            db = _db;
...
//                if (shopId != 0)
//                {
//                    var wcDetails = db.Tblmachinedetails.Where(x => x.IsDeleted == 0 && x.ShopId == shopId).Select(x => new { x.MachineId, x.MachineInvNo }).ToList();
...
//                        var updatePcpNo = db.TblPcpNo.Where(m => m.PartNo == PartInner).Select(m => m.PcpNo).FirstOrDefault();
//                        if(updatePcpNo != null)
//                        {
//                            ddldataInner.PcpNo = updatePcpNo;
//                            db.SaveChanges();
//                        }

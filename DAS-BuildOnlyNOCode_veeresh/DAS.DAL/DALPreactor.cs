...
//    {
//        public i_facility_tsalContext db = new i_facility_tsalContext();
//        private readonly AppSettings appSettings;
//        private static readonly log4net.ILog log = log4net.LogManager.GetLogger(typeof(DALPreactor));

//        public DALPreactor(i_facility_tsalContext _db, IOptions<AppSettings> _appSettings)
//        {
//            db = _db;
...
//                    using (SqlConnection sqlConn = new SqlConnection(connectionString1))
//                    {
//                        string sql = "Truncate Table [i_facility_tsal].[dbo].[tblactivity]";
//                        using (SqlCommand sqlCmd = new SqlCommand(sql, sqlConn))
//                        {
//                            sqlConn.Open();
//                            sqlCmd.ExecuteNonQuery();
...
//                    foreach (var dataitem in data.activitylist)
//                    {
//                        var plantid = db.Tblplant.Where(m => m.PlantName == dataitem.plantName && m.IsDeleted == 0).Select(m => m.PlantId).FirstOrDefault();
//                        if (plantid != 0)
//                        {
//                            var shopid = db.Tblshop.Where(m => m.ShopName == dataitem.shopName && m.IsDeleted == 0).Select(m => m.ShopId).FirstOrDefault();
//                            if (shopid != 0)
//                            {
//                                var cellid = db.Tblcell.Where(m => m.CellName == dataitem.cellName && m.IsDeleted == 0).Select(m => m.CellId).FirstOrDefault();
//                                if (cellid != 0)
//                                {
//                                    var machineId = db.Tblmachinedetails.Where(m => m.MachineInvNo == dataitem.machineName && m.IsDeleted == 0).Select(m => m.MachineId).FirstOrDefault();
//                                    if (machineId != 0)
//                                    {
//                                        var procid = db.TblProcess.Where(m => m.ProcessName == dataitem.processName).Select(m => m.ProcessId).FirstOrDefault();
//                                        if (procid != 0)
...
//                    using (SqlConnection sqlConn = new SqlConnection(connectionString1))
//                    {
//                        string sql = "Truncate Table [i_facility_tsal_New].[dbo].[scrapQty]";
//                        using (SqlCommand sqlCmd = new SqlCommand(sql, sqlConn))
//                        {
//                            sqlConn.Open();
//                            sqlCmd.ExecuteNonQuery();
...
//                    foreach (var item in data.ScrapqntyList)
//                    {
//                        var check = db.ScrapQtyDet.Where(m => m.WorkOder == item.order).FirstOrDefault();
//                        if (check == null)
//                        {
...
//                            int cclgcong = Convert.ToInt32(item.ccldConf);
//                            int scrapquantity = Convert.ToInt32(item.scrapQuantity);
//                            ScrapQtyDet tblScrapqnt = new ScrapQtyDet();
...
//                            db.ScrapQtyDet.Add(tblScrapqnt);
//                            db.SaveChanges();
//                            obj.isStatus = true;
//                            obj.isTure = true;
//                            obj.response = ResourceResponse.AddedSuccessMessage;
...
//            try
//            {
//                var check = (from wf in db.ScrapQtyDet
//                             select new
//                             {

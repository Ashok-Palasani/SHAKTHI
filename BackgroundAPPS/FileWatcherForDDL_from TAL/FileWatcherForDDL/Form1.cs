        ...
        public Form1()
        {
            try
            {
               ...
               var nwDetails = db.tblnetworkdetailsforddls.Where(m => m.IsDeleted == 0).FirstOrDefault();
                ...
                var nwDetails1 = db.tblnetworkdetailsforddls.Where(m => m.IsDeleted == 0 && m.NPFDDLID == 2).FirstOrDefault(); // INCremental DDL
       ...
       private void fileSystemWatcher1_Created(object sender, FileSystemEventArgs e)
        {
            ...
            try
            {
                ...
                IntoFile("DDL Extension" + fileExt);
                if ((!filename.Contains("~$")) || (fileExt == ".CSV") || (fileExt == ".csv"))
                {
                    //IntoFile("File Extension" + fileExt);
                    string destinationPathWithFileName = @"\\192.168.0.2\Ifacility_IN\Uploaded DDL\" + filename; //server

                    //string destinationPathWithFileName = @"D:\PCS-DAS-Sync\Uploaded DDL\" + filename; //local
                    ...
                    try
                    {
                        ...
                        //string conString = "Data Source = DESKTOP-72HGDFG\\SQLDEV17013; User ID = sa;Password = srks4$;Initial Catalog = i_facility_shakti;Persist Security Info=True";
       ...
       private void fileSystemWatcher2_Created(object sender, FileSystemEventArgs e)
        {
            try
            {
                ...
                string fileExt = Path.GetExtension(rootFolderPath);
                IntoFile("fileExt");
                //if ((!filename.Contains("~$")) || (fileExt == ".xlsx")|| (fileExt == ".csv") || (fileExt == ".CSV"))
                if ((!filename.Contains("~$")) || (fileExt == ".CSV") || (fileExt == ".csv"))
                {
                    string destinationPathWithFileName = @"\\192.168.0.2\Ifacility_IN\Uploaded Incremental DDL\" + filename;
        ...
        public void HandleDDLFile(string DDLPath)
        {
            try
            {
                ...
                if (f.Length > 0)
                {
                    ...
                    //string fileLocation = @"C:\Program Files(x86)\SRKS Software Solutions Pvt. Ltd\Filewatcher\DDL\" + filename;
                    ...
                    try
                    {
                        ...
                        dt = ReadCsvFile(fileLocation);
                        IntoFile("CSV Data readeed");
                    }
                    ...
                    try
                    {
                        ...
                        //    "DELETE from i_facility_shakti.dbo.tblddl where IsCompleted = 0 or CorrectedDate <= '" + System.DateTime.Now.AddDays(-2).ToString("yyyy-MM-dd") + "' ;" +
        ...
        public void CheckDDLStatus(int i)
        {
            ...
            //if (i == 1)
            //{
            //    string statusQuery = "Truncate table [i_facility_shakti].[dbo].[tblDDLStatus]";
            //    MsqlConnection con = new MsqlConnection();
            //    con.open();
            //    using (SqlConnection databaseConnection = new SqlConnection(con.msqlConnection.ConnectionString))
            //    {
            ...
            if (i == 1)
            {
                using (MsqlConnection mc1 = new MsqlConnection())
                {
                    ...
                    string statusQuery = "Truncate table [i_facility_shakti].[dbo].[tblDDLStatus]";
                        ...
                        SqlCommand cmd = new SqlCommand(statusQuery, mc1.msqlConnection);
                        cmd.ExecuteNonQuery();
                    mc1.close();
                }
            }
            else
            {
                using (MsqlConnection mc1 = new MsqlConnection())
                {
                    ...
                    string queryInsert = "INSERT INTO [i_facility_shakti].[dbo].[tblDDLStatus] ([StatusMessage],[Status],[CreatedOn],[CreatedBy],[IsDeleted])VALUES('DDL file Uploading', 1, '" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "', 1, 0)";
        ...
        public DataTable ReadCsvFile(string path, bool hasHeader = true)
        {
                            ...
                            string[] rowValues = rows[i].Split(','); //split each row with comma to get individual values  
                            {
                                if ((i == 0))
                                {
                                    if (i == 0 && Testvalue == 0)
                                    {
                                        ...
                                        {
                                            string Columnname = rowValues[j];
                                            //Columnname = GetColumname(rowValues[j]);
                                            if (Columnname.Contains("\r"))
                                            {
                                                Columnname = Columnname.Replace('\r', ' ').TrimEnd();
                                            }
                                ...
                                else
                                {
                                    ...
                                    for (int k = 0; k < rowValues.Count(); k++)
                                    {
                                        ...
                                        string val4 = Regex.Replace(rowValues[4], "[@,\\.\";'\\\\]", string.Empty);
                                        string val6 = Regex.Replace(rowValues[6], "[@,\\.\";'\\\\]", string.Empty);
                                        if ((val4 == val6) && k == 4)
                                        {
                                            rowValues = rowValues.Where((source, index) => index != 6).ToArray();
                                            if (value.Contains("\r"))
                                            {
                                                value = value.Replace('\r', ' ').TrimEnd();
                                            }
                                            dr[k] = value.ToString();
                                        }
                                        else
            ...
            {
                IntoFile("ROW :" + rowval + ex.ToString());
            }

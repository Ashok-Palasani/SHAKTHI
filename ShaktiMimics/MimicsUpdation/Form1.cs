...
namespace MimicsUpdation
{
    ...
    {
        i_facility_tealEntities db = new i_facility_tealEntities();
        ...
        {
            using (i_facility_tealEntities db = new i_facility_tealEntities())
            {
                List<tblmachinedetail> machinedetails = db.tblmachinedetails.Where(m => m.IsDeleted == 0 && m.IsNormalWC == 0).ToList();
        ...
        private void UpdateMimicsdetails(List<tblmachinedetail> machines, DateTime correctedDate)
        {
            ...
            {
                IntoFile("machineID:"+row.MachineID);
                ...
                using (i_facility_tealEntities db = new i_facility_tealEntities())
                {
                    GetModeDurations = db.tbllivemodes.Where(m => m.MachineID == MachineID && m.CorrectedDate == correcteddate && m.IsCompleted == 1).ToList();
                }
                ...
                using (i_facility_tealEntities db = new i_facility_tealEntities())
                {
                    GetModeDurationsRunning = db.tbllivemodes.Where(m => m.MachineID == MachineID && m.CorrectedDate == correcteddate && m.IsCompleted == 0).ToList();
                }
        ...
        private void GetPartsandCutting()
        {
            ...
            foreach (var machine in machinedet)
            {
                ...
                using (i_facility_tealEntities db1 = new i_facility_tealEntities())
                {
                    scrap = db1.tblworkorderentries.Where(m => m.MachineID == machineid && m.CorrectedDate == correcteddate).OrderByDescending(m => m.HMIID).FirstOrDefault();  //workorder entry
                    if (scrap != null)
                    {
        ...
        {           
            ...
            if (endtime <= temp)
            {
                ...
                {
                    int PartPerCycle = 1;
                    var WorkOrderEntryDet = db.tblworkorderentries.Where(m => m.IsStarted == 1 && m.IsFinished == 0 && m.CorrectedDate == CrctDate && m.MachineID == machineid).FirstOrDefault(); // to mutiple by partpercycle
                    if(WorkOrderEntryDet!=null)
                    {
                        PartPerCycle =(int) WorkOrderEntryDet.PartsPerCycle;
                    }
                    ...
                    if (edtime <= temp)
                    {
                        ...
                        if (LastRow != null && FirstRow != null)
                        {
                            ...
                            if (FirstRow.PartsTotal > LastRow.PartsTotal)
                            {
                                using (i_facility_tealEntities db1 = new i_facility_tealEntities())
                                {
                                    ...
                                    var prevrecord = db1.parameters_master.Where(m => m.ParameterID < Zero && m.MachineID == machineid).OrderByDescending(m => m.ParameterID).Select(m => m.PartsTotal).FirstOrDefault();
                        ...
                        if (parts_cutting == null)
                        {
                            ...
                            if(PartPerCycle>1)
                            {
                                parts.PartsPerCyscleEnteredTime = WorkOrderEntryDet.WOStart;
        ...
        private string ShiftDet()
        {
            ...
            if(ShiftValue=="")
            {
                ShiftValue= db.tblshift_mstr.Where(m => m.IsDeleted == 0).OrderByDescending(m=>m.ShiftID).Select(m=>m.ShiftName).FirstOrDefault();
            }
        ...
        public void IntoFile(string Msg)
        {
            try
            {
                string appPath = "C:/SRKSLog/mimicslog.txt";

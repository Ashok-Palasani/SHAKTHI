...
namespace DAS.DAL
{
    ...
    {
        public i_facility_shaktiContext db = new i_facility_shaktiContext();
        private static readonly log4net.ILog log = log4net.LogManager.GetLogger(typeof(DALLoss));
        public static IConfiguration configuration;

        public DALLoss(i_facility_shaktiContext _db, IConfiguration _configuration)
        {
            db = _db;
        ...
        //                    var machineName = db.Tblmachinedetails.Where(x => x.IsDeleted == 0 && x.MachineId == machineId).Select(x => new { x.MachineName, x.PlantId, x.ShopId, x.CellId }).FirstOrDefault();
        ...
        //                    objLossdetData.machineNmae = machineName.MachineName;
        ...
        //        var machinedet = (from s in db.Tblmachinedetails where (s.IsDeleted == 0 && s.CellId == loss.cellId) select new { machineid = s.MachineId, machinename = s.MachineName }).ToList();

        //        if (machineId != 0)
        //        {
        //            machinedet = (from s in db.Tblmachinedetails where (s.IsDeleted == 0 && s.MachineId == machineId) select new { machineid = s.MachineId, machinename = s.MachineName }).ToList();

        //        }
        //        else if (cellId != 0)
        //        {
        //            machinedet = (from s in db.Tblmachinedetails where (s.IsDeleted == 0 && s.CellId == cellId) select new { machineid = s.MachineId, machinename = s.MachineName }).ToList();

        //        }
        //        else if (shopId != 0)
        //        {
        //            machinedet = (from s in db.Tblmachinedetails where (s.IsDeleted == 0 && s.ShopId == shopId) select new { machineid = s.MachineId, machinename = s.MachineName }).ToList();

        //        }
        //        else if (plantId != 0)
        //        {
        //            machinedet = (from s in db.Tblmachinedetails where (s.IsDeleted == 0 && s.PlantId == plantId) select new { machineid = s.MachineId, machinename = s.MachineName }).ToList();
        ...
        public EntityModelWithLossCount GetLossDet(lossmodel loss)
        {
            ...
            try
            {
                ...
                DateTime fromD =Convert.ToDateTime( loss.FromDate);
                string fromdatee = fromD.ToString("yyyy-MM-dd");
                ...
                foreach (var item in machinedet)
                {
                    ...
                    var tcfLossEntry = db.Tbltcflossofentry.Where(x => x.IsAccept == 0 && x.MachineId == item.machineid && x.CorrectedDate == fromdatee /*loss.FromDate*/).ToList();
                    if (tcfLossEntry.Count == 0)
                    {


                        // chenaged tbl from Tbllossofentry to Tbllivelossofentry
                        var lossdet = (from s in db.Tbllivelossofentry where (s.MachineId == item.machineid && (DateTime.Parse(s.CorrectedDate) == DateTime.Parse(loss.FromDate))/*s.MessageCodeId == 999*/) select new { EndTime = s.EndDateTime, StartTime = s.StartDateTime, Lossid = s.LossId, s.MessageCodeId, CorrectedDate = s.CorrectedDate }).ToList();
                    ...
                    else if (tcfLossEntry.Count > 0)
                    {
                        ...
                        tcfLossEntry = db.Tbltcflossofentry.Where(x => x.IsAccept == 0 && x.MachineId == item.machineid && x.CorrectedDate == fromdatee).ToList();
        ...
        public bool InsertIntoBackUpTable(int machineId, string correctedDate)
        {
            ...
            try
            {
                DateTime fromD = Convert.ToDateTime(correctedDate);
                string fromdatee = fromD.ToString("yyyy-MM-dd");


                var tcflossBackup = db.TblBachUplossofentry.Where(m => m.MachineId == machineId && m.CorrectedDate == fromdatee).ToList();
                if (tcflossBackup.Count == 0)
                {
                    var lossdet = (from s in db.Tbllivelossofentry where (s.MachineId == machineId && (DateTime.Parse(s.CorrectedDate) == DateTime.Parse(correctedDate))/* && s.MessageCodeId == 999*/) select s).ToList();
        ...
        public EntityModel GetLossCodeLevel1()
        {
            ...
            try
            {
               ...
               // var lossdet = db.Tbllossescodes.Where(m => m.IsDeleted == 0 && m.LossCodesLevel == 1 && m.LossCode != "999" && m.LossCodeDesc != "Breakdown").ToList();
                ...
                var lossdet = db.Tbllossescodes.Where(m => m.IsDeleted == 0 && m.LossCodesLevel == 1 && m.LossCodeDesc != "Breakdown").ToList();
        ...
        public EntityModel SendToApproveAllLossDetails(EntityHMIDetails data)
        {
            ...
            try
            {
                ...
                string machName = "";
                var machineData = db.Tblmachinedetails.Where(x => x.IsDeleted == 0).Select(x => new { x.MachineId, x.MachineName }).ToList();
                if (data.machineId != 0)
                {
                    machineData = db.Tblmachinedetails.Where(x => x.IsDeleted == 0 && x.MachineId == machId).Select(x => new { x.MachineId, x.MachineName }).ToList();
                    machName = "No Code " + db.Tblmachinedetails.Where(x => x.IsDeleted == 0 && x.MachineId == machId).Select(x => x.MachineName).FirstOrDefault() + " " + correctedDate;
                }
                else if (data.cellId != 0)
                {
                    machineData = db.Tblmachinedetails.Where(x => x.IsDeleted == 0 && x.CellId == cellId).Select(x => new { x.MachineId, x.MachineName }).ToList();
                ...
                {
                    machineData = db.Tblmachinedetails.Where(x => x.IsDeleted == 0 && x.ShopId == shopId).Select(x => new { x.MachineId, x.MachineName }).ToList();
                ...
                {
                    machineData = db.Tblmachinedetails.Where(x => x.IsDeleted == 0 && x.PlantId == plantId).Select(x => new { x.MachineId, x.MachineName }).ToList();
                ...
                foreach (var machineRow in machineData)
                {
                    ...
                    if (tcfloss.Count > 0)
                    {
                        ...
                        foreach (var row in tcfloss)
                        {
                            ...
                            string machineName = db.Tblmachinedetails.Where(x => x.IsDeleted == 0 && x.MachineId == row.MachineId).Select(x => x.MachineName).FirstOrDefault();
                ...
                foreach (var machineRow in machineData)
                {
                    ...
                    if (data.id == "")
                    {
                        ...
                        if (NoCodeDet.Count > 0)
                        {
                            ...
                            {
                                var machineDetais = db.Tblmachinedetails.Where(x => x.IsDeleted == 0 && x.MachineId == row.MachineId).Select(x => new { x.MachineName, x.IsNormalWc }).FirstOrDefault();
                                ...
                                reaobj.MachineName = machineRow.MachineName;
                    ...
                    else
                    {
                        ...
                        if (ret == true)
                        {
                            foreach (var rowid in ids)
                            {
                                ...
                                if (NoCodeDet != null)
                                {
                                    ...
                                    //foreach (var row in NoCodeDet)
                                    //{
                                    var machineDetais = db.Tblmachinedetails.Where(x => x.IsDeleted == 0 && x.MachineId == NoCodeDet.MachineId).Select(x => new { x.MachineName, x.IsNormalWc }).FirstOrDefault();
                                    getReasonTable reaobj = new getReasonTable();
                        ...
                        //        {
                        //            var machineDetais = db.Tblmachinedetails.Where(x => x.IsDeleted == 0 && x.MachineId == norow.MachineId).Select(x => new { x.MachineName, x.IsNormalWc }).FirstOrDefault();
                        //            getReasonTable reaobj = new getReasonTable();
        ...
        public bool InsertToOEEDashboardVar(Tbloeedashboardvariables oeeDasjboardVar)
        {
            ...
            try
            {
                if (oeeDasjboardVar != null)
                {
                    ...
                    addRow.Blue = Convert.ToDouble(oeeDasjboardVar.Blue);
                    addRow.CellId = oeeDasjboardVar.CellId;
                    addRow.CreatedBy = oeeDasjboardVar.CreatedBy;
                    addRow.CreatedOn = oeeDasjboardVar.CreatedOn;
                    addRow.DownTimeBreakdown = Convert.ToDouble(oeeDasjboardVar.DownTimeBreakdown);
                    addRow.EndDate = oeeDasjboardVar.EndDate;
                    addRow.Green = Convert.ToDouble(oeeDasjboardVar.Green);
                    ...
                    addRow.MinorLosses = Convert.ToDouble(oeeDasjboardVar.MinorLosses);
                    //addRow.OeevariablesBackupId = oeeDasjboardVar.OeevariablesId;
                    addRow.PlantId = oeeDasjboardVar.PlantId;
                    addRow.ReWotime = Convert.ToDouble(oeeDasjboardVar.ReWotime);
                    addRow.Roalossess = Convert.ToDouble(oeeDasjboardVar.Roalossess);
                    addRow.ScrapQtyTime = Convert.ToDouble(oeeDasjboardVar.ScrapQtyTime);
                    addRow.SettingTime = Convert.ToDouble(oeeDasjboardVar.SettingTime);
                    addRow.ShopId = oeeDasjboardVar.ShopId;
                    addRow.StartDate = oeeDasjboardVar.StartDate;
                    addRow.SummationOfSctvsPp = Convert.ToDouble(oeeDasjboardVar.SummationOfSctvsPp);
        ...
        //                    //using (i_facility_shaktiContext dbLoss = new i_facility_shaktiContext())
        //                    //{
        ...
        //    //using (i_facility_shaktiContext dbhmi = new i_facility_shaktiContext())
        //    //{
        ...
        //        //using (i_facility_shaktiContext db = new i_facility_shaktiContext())
        //        //{
        ...
        //                                        //Now insert into table
        //                                        using (i_facility_shaktiContext db1 = new i_facility_shaktiContext())
        //                                        {
        //                                            try
        //                                            {
        //                                                Tblwolossess objwo = new Tblwolossess();
        ...
        //        using (i_facility_shaktiContext dblivedps = new i_facility_shaktiContext())
        //        {

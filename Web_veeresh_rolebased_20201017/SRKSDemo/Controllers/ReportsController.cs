        ...
        public void UtilizationReportExcel(string StartDate, string EndDate, string PlantID, string ShopID = null, string CellID = null, string WorkCenterID = null, string TabularType = null)
        {
            ...
            if (string.IsNullOrEmpty(WorkCenterID))
            {
                if (string.IsNullOrEmpty(CellID))
                {
                    if (string.IsNullOrEmpty(ShopID))
                    {
                        ...
                        else
                        {
                            ...
                            MacCount = obj.GettbMachineDetails(plantId).ToList().Count();
                            //   var plantName = obj2.GettbplantDet(plantId);
                            string plantName = obj2.GettbplantDet(plantId);
                    ...
                    else
                    {
                        ...
                        string shopName = obj2.GettbShopIDDet(shopId);
                ...
                else
                {
                    ...
                    // MacCount = obj.GettbMachineDetails2(cellId).ToList().Count();
                    string cellName = obj2.GettbcellDet(cellId);
            ...
            else
            {
                ...
                MacCount = 1;
                string WCName = obj2.GettbMachineDe1t(wcId);
            ...
            do
            {
                ...
                DateTime testDate = UsedDateForExcel;
                ...
                for (int n = 0; n < machin.Rows.Count; n++)
                {
                    ...
                    double green, blue = 0, OperatingTime = 0, DownTimeBreakdown = 0, ROALossess = 0, AvailableTime = 0, SettingTime = 0;
                    ...
                    using (MsqlConnection mcOEE = new MsqlConnection())
                    {
                        mcOEE.open();
                        string query = null;
                        query = @"select Count(WCID),sum(Green), sum(SummationOfSCTvsPP),sum(SettingTime),sum(ROALossess),SUM(MinorLosses), sum(Blue), sum(DownTimeBreakdown), "
                                + " sum(ScrapQtyTime),sum(ReWOTime) from " + MsqlConnection.DbName + ".tbloeedashboardvariables where StartDate >= '" + correctedDateS + "' and StartDate <= '" + correctedDateE + "' and WCID = '" + MachineID + "' ;";
                    ...
                    double ROAMainLosses = Math.Round(((ROALossess - SettingTime) ), 1);
                    ...
                    worksheet.Cells["H" + Row].Value = Math.Round((OperatingTime ), 1);
                    worksheet.Cells["I" + Row].Value = Math.Round((SettingTime ), 1);
                    worksheet.Cells["J" + Row].Value = ROAMainLosses;
                    worksheet.Cells["K" + Row].Value = Math.Round((MinorLosses), 1);
                    worksheet.Cells["L" + Row].Value = Math.Round((DownTimeBreakdown), 1);
                ...
                if (lowestLevel != "WorkCentre")
                {
                    ...
                    double blah = Math.Round((IndividualDateValueAdding / (24 * 60 * machin.Rows.Count)) * 100, 1);
                ...
                if (Row > thisDatesDataStartsAt)
                {
                    ...
                    ExcelRange modelTable = worksheet.Cells["B" + thisDatesDataStartsAt + ":O" + Row];
                    // Assign borders
            ...
            ExcelRange modelTableUtil = worksheet.Cells["B6:O" + Row];
            // Assign borders
            ...
            if (DTUtilDayWise.Rows.Count > 0)
            {
                ...
                ExcelBarChart chartIDAndUnID = (ExcelBarChart)worksheetOA.Drawings.AddChart("Utilization", eChartType.ColumnStacked);
                ...
                ExcelChartSerie ValueAdding = chartIDAndUnID.Series.Add(erValueAddingValues, erDateNAMES);
                ValueAdding.Header = "ValueAdding";
                ExcelChartSerie Setting = chartIDAndUnID.Series.Add(erSettingValues, erDateNAMES);
                Setting.Header = "Setting";
                ExcelChartSerie Idle = chartIDAndUnID.Series.Add(erIdleValues, erDateNAMES);
                Idle.Header = "Idle";
                ExcelChartSerie MinorLoss = chartIDAndUnID.Series.Add(erMinorLossValues, erDateNAMES);
                MinorLoss.Header = "MinorLoss";
                ExcelChartSerie Breakdown = chartIDAndUnID.Series.Add(erBreakdownValues, erDateNAMES);
                Breakdown.Header = "Breakdown";
                ExcelChartSerie NoPlan = chartIDAndUnID.Series.Add(erNoPlanValues, erDateNAMES);
                ...
                ExcelChart chartTypeL1 = chartIDAndUnID.PlotArea.ChartTypes.Add(eChartType.Line);
                ExcelChartSerie serieL2 = chartTypeL1.Series.Add(worksheetOA.Cells[Col2 + "50:" + Col2 + (50 + DTUtilDayWise.Rows.Count - 1)], worksheetOA.Cells[Col1 + "50:" + Col1 + (50 + DTUtilDayWise.Rows.Count - 1)]);



                //Get reference to the worksheet xml for proper namespace
                XmlDocument chartXml = chartIDAndUnID.ChartXml;
                ...
                XmlNodeList valAxisNodes = chartXml.SelectNodes("c:chartSpace/c:chart/c:plotArea/c:valAx", nsm);
                if (valAxisNodes != null && valAxisNodes.Count > 0)
                {
                    foreach (XmlNode valAxisNode in valAxisNodes)
                    {
                        XmlNode major = valAxisNode.SelectSingleNode("c:majorGridlines", nsm);
                        if (major != null)
                        {
                            valAxisNode.RemoveChild(major);
                        }

                        XmlNode minor = valAxisNode.SelectSingleNode("c:minorGridlines", nsm);
                        if (minor != null)
                        {
                ...
                XmlNodeList catAxisNodes = chartXml.SelectNodes("c:chartSpace/c:chart/c:plotArea/c:catAx", nsm);
                if (catAxisNodes != null && catAxisNodes.Count > 0)
                {
                    foreach (XmlNode catAxisNode in catAxisNodes)
                    {
                        XmlNode major = catAxisNode.SelectSingleNode("c:majorGridlines", nsm);
                        if (major != null)
                        {
                            catAxisNode.RemoveChild(major);
                        }

                        XmlNode minor = catAxisNode.SelectSingleNode("c:minorGridlines", nsm);
                        if (minor != null)
                        {

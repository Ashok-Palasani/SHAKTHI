...
namespace DAS.DAL
{
    ...
    {
        i_facility_shaktiContext db = new i_facility_shaktiContext();

        private static readonly log4net.ILog log = log4net.LogManager.GetLogger(typeof(DALCommonMethod));

        public static IConfiguration configuration;

        public DALCommonMethod(i_facility_shaktiContext _db, IConfiguration _configuration)
        {
            db = _db;
                        ...
                        try
                        {
                            ...
                            if (OEEDataPresent.Count == 0)
                            {
                                ...
                                try
                                {
                                    ...
                                    objoee.StartDate = UsedDateForExcel;
                                    objoee.EndDate = UsedDateForExcel;
                                    objoee.MinorLosses = Convert.ToDecimal(Math.Round(MinorLosses / 60, 2));
                                    objoee.Blue = Convert.ToDecimal(Math.Round(blue / 60, 2));
                                    objoee.Green = Convert.ToDecimal(Math.Round(green / 60, 2));
                                    objoee.SettingTime = Convert.ToDecimal(Math.Round(SettingTime, 2));
                                    objoee.Roalossess = Convert.ToDecimal(Math.Round(ROALossess / 60, 2));
                                    objoee.DownTimeBreakdown = Convert.ToDecimal(Math.Round(DownTimeBreakdown, 2));
                                    objoee.SummationOfSctvsPp = Convert.ToDecimal(Math.Round(SummationOfSCTvsPP, 2));
                                    objoee.ScrapQtyTime = Convert.ToDecimal(Math.Round(ScrapQtyTime, 2));
                                    objoee.ReWotime = Convert.ToDecimal(Math.Round(ReWOTime, 2));
                                    objoee.Loss1Name = lossCode1;
                                    objoee.Loss1Value = lossCodeVal1;
                                    objoee.Loss2Name = lossCode2;
                                    objoee.Loss2Value = lossCodeVal2;
